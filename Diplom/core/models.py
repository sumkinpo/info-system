...
class Entity(models.Model):
    ...
    begin_date = models.DateField(verbose_name=_('Дата начала'))
    end_date = models.DateField(verbose_name=_('Дата окончания'), blank=True, null=True, default=None)
    ...
    main_image = models.ForeignKey(
        ...
        default=None,
        on_delete=models.SET_DEFAULT,
        related_name=_('Объект'),
    )

    def save(self, force_insert=False, force_update=False, using=None,
             update_fields=None):
        if self.end_date and self.begin_date > self.end_date:
            raise ValueError(_('Дата начала не может быть меньше даты конца'))
        else:
...
class Author(models.Model):
    ...
    def save(self, force_insert=False, force_update=False, using=None,
             update_fields=None):
        if self.end_date and self.begin_date > self.end_date:
            raise ValueError(_('Конечная дата не может быть меньше начальной'))
        else:
